...
export class MemorizationService {
  ...
  private apiUrl = `${environment.apiUrl}/Memorization`;

  add(dto: MemorizationRecordCreateDTO): Observable<MemorizationRecordDTO> {
    return this.http.post<MemorizationRecordDTO>(this.apiUrl, dto);
  }

  update(id: number, dto: MemorizationRecordCreateDTO): Observable<void> {
  ...
  // List of Quran Surahs in Arabic
  surahs = [
  ...
  getSurahName(id: number): string {
    return this.surahs.find(s => s.id === id)?.name || 'غير معروف';
  }
}
